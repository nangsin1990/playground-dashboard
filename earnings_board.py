...
def _today_gap(px: pd.DataFrame, next_date: str | None, past_dates: list) -> float | None:
    if px is None or len(px) < 2:
        return None
    last_day = pd.Timestamp(px.index[-1]).normalize()
    prev_close = float(px["Close"].iloc[-2])
    last_open = float(px["Open"].iloc[-1])
    if prev_close <= 0:
        return None
    marks = []
    if next_date:
...
def fetch_earnings_board() -> dict:
    try:
        ...
        return {"ok": False, "error": "earnings_board_unavailable"}
