...
FETCH_STATE = {"stage": "idle", "markets": {}}
_lock = threading.Lock()

def _refresh_aggregate_locked():
    markets = FETCH_STATE.get("markets") or {}
    if not markets:
        return
    done = sum(int(m.get("tickers_done_market") or 0) for m in markets.values())
    total_mkt = sum(int(m.get("tickers_total_market") or 0) for m in markets.values())
    FETCH_STATE["tickers_done"] = done
    if total_mkt:
        FETCH_STATE["tickers_total"] = total_mkt
    active = [name for name, m in markets.items() if not m.get("done")]
    if active:
        current = active[-1]
        snap = markets[current]
        FETCH_STATE["market"] = current
        FETCH_STATE["batch"] = snap.get("batch", 0)
        FETCH_STATE["total_batches"] = snap.get("total_batches", 0)
        FETCH_STATE["tickers_done_market"] = snap.get("tickers_done_market", 0)
        FETCH_STATE["tickers_total_market"] = snap.get("tickers_total_market", 0)

def _upd(**kwargs):
    with _lock:
        FETCH_STATE.update(kwargs)
        _refresh_aggregate_locked()

def get_fetch_state():
    with _lock:
        state = dict(FETCH_STATE)
        state["markets"] = {k: dict(v) for k, v in (FETCH_STATE.get("markets") or {}).items()}
        state["markets_done"] = list(FETCH_STATE.get("markets_done") or [])
        return state
...
def _with_personal(active: dict) -> dict:
    ...
    for t in PERSONAL_TICKERS:
        if t in existing:
            continue
        extra[t] = PERSONAL_META.get(t, (t, "Unknown"))
    if not extra:
        return active
...
def _fetch_market(mkt_name: str, tickers_dict: dict) -> tuple[str, dict]:
    ...
    with _lock:
        markets = FETCH_STATE.setdefault("markets", {})
        markets[mkt_name] = {
            "batch": 0,
            "total_batches": total_batches,
            "tickers_done_market": 0,
            "tickers_total_market": len(tickers),
            "done": False,
        }
        _refresh_aggregate_locked()
    ...
    for i, batch in enumerate(data_io.chunk(tickers, PIPELINE_BATCH_SIZE)):
        t0 = time.time()
        with _lock:
            mstate = FETCH_STATE.setdefault("markets", {}).setdefault(mkt_name, {})
            mstate["batch"] = i + 1
            _refresh_aggregate_locked()
        ...
        with _lock:
            mstate = FETCH_STATE.setdefault("markets", {}).setdefault(mkt_name, {})
            mstate["tickers_done_market"] = int(mstate.get("tickers_done_market") or 0) + len(batch)
            _refresh_aggregate_locked()
        ...
        try:
            for future in concurrent.futures.as_completed(futures_map, timeout=300):
                ...
                try:
                    ...
                    with _lock:
                        done = list(FETCH_STATE.get("markets_done") or [])
                        if mkt_name not in done:
                            done.append(mkt_name)
                        FETCH_STATE["markets_done"] = done
                        mstate = FETCH_STATE.setdefault("markets", {}).setdefault(mkt_name, {})
                        mstate["done"] = True
                        _refresh_aggregate_locked()
...
def compute_dashboard(combined, ticker_meta, fetch_results, active) -> dict:
        ...
        try:
            ...
            for k, v in last_rolled.items():
                if v and k in signal_count_5d:
                    signal_count_5d[k] += 1
            ticker_signal[t] = {"rolled": last_rolled, "count": int(count_val), "confluence": conf_val, "vcp": vcp}
        except Exception:
    ...
    for t in watch:
        ...
        watchlist.append({
            ...
            "price": round(float(d["Close"].iloc[-1]), 4 if float(d["Close"].iloc[-1]) < 1 else 2),
    ...
    expected = sum(len(v) for v in (active or {}).values()) or len(ticker_meta)
    loaded = len(combined)
    coverage = (loaded / expected * 100) if expected else 0.0
    if loaded == 0:
        yahoo_status = "down"
    elif coverage >= 95:
        yahoo_status = "ok"
    ...
    missing_pct = max(0.0, 100.0 - coverage)
    quality = {
        "expected_tickers": expected,
        "loaded_tickers": loaded,
        "coverage_pct": round(coverage, 1),
        "missing_pct": round(missing_pct, 1),
        "per_market": (sync or {}).get("markets") or [],
        "source": "yahoo",
        "source_status": yahoo_status,
        "stale": False,
        "confidence": "high" if coverage >= 95 else ("medium" if coverage >= 70 else "low"),
    }
    _upd(stage="done")
    return {
        "ok": True, "updated": now_str, "universe_total": expected, "universe_loaded": loaded,
        ...
        "stat_cards": {"VDU": signal_count_5d["VDU"], "PPBP": signal_count_5d["PPBP"], "BGU": signal_count_5d["BGU"],
                       ...
                       "total": sum(signal_count_5d.values())},
        "feed_status": {
            "yahoo": yahoo_status == "ok",
