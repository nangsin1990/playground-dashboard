...
def compute_dashboard(combined, ticker_meta, fetch_results, active) -> dict:
    ...
    def _compute_signal_for(item):
        ...
        try:
            ...
            return t, {"rolled": last_rolled, "count": int(count_val), "confluence": conf_val, "vcp": vcp}, None
        except Exception as e:
            return t, {"rolled": {}, "count": 0, "confluence": False}, e
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=sig_workers) as sig_ex:
        for t, result, err in sig_ex.map(_compute_signal_for, combined.items()):
            ...
            for k, v in result["rolled"].items():
    ...
    return {
        ...
        "feed_status": {"yahoo": True, "updated": now_str, "loaded": len(combined), "total": len(ticker_meta)},
