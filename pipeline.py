...
import data_io
from universe import FLAGS, UNIVERSE, LISTED_CORE, LISTED_FULL, universe_list_stats
...
def _upd_market(mkt_name: str, **kwargs):
    with _lock:
        markets = dict(FETCH_STATE.get("markets") or {})
        row = dict(markets.get(mkt_name) or {})
        row.update(kwargs)
        markets[mkt_name] = row
        FETCH_STATE["markets"] = markets
        FETCH_STATE["market"] = mkt_name
        FETCH_STATE["batch"] = row.get("batch", 0)
        FETCH_STATE["total_batches"] = row.get("total_batches", 0)
        FETCH_STATE["tickers_done_market"] = row.get("tickers_done", 0)
        FETCH_STATE["tickers_total_market"] = row.get("tickers_total", 0)
        FETCH_STATE["tickers_done"] = sum(int(m.get("tickers_done") or 0) for m in markets.values())
...
def load_market_pack(mode: str) -> dict:
    ...
    try:
        ...
        dash = {
            ...
            "universe_listed_core": LISTED_CORE,
            "universe_listed_full": LISTED_FULL,
        }
...
def _fetch_market(mkt_name: str, tickers_dict: dict) -> tuple[str, dict]:
    ...
    _upd_market(mkt_name, batch=0, total_batches=total_batches, tickers_done=0, tickers_total=len(tickers))

    for i, batch in enumerate(data_io.chunk(tickers, PIPELINE_BATCH_SIZE)):
        t0 = time.time()
        _upd_market(mkt_name, batch=i + 1)
        batch_results = data_io.fetch_batch(tuple(batch))
        results.update({t: v for t, v in batch_results.items() if v is not None})
        done = min(len(tickers), (i + 1) * PIPELINE_BATCH_SIZE)
        _upd_market(mkt_name, tickers_done=done)
...
def fetch_universe(active: dict) -> tuple[dict, dict, dict]:
    ...
    t0 = time.time()
    _upd(stage="fetching", started=t0, tickers_total=total, markets_total=markets, markets_done=[], tickers_done=0, markets={})
...
def compute_dashboard(combined, ticker_meta, fetch_results, active) -> dict:
    ...
    # (โหมด full ~1071 ตัวในลิสต์) ตรงนี้เป็นคอขวดที่ 2 รองจาก fetch เพราะรัน rolling indicator
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=sig_workers) as sig_ex:
        for t, result, err in sig_ex.map(_compute_signal_for, combined.items()):
            if err is not None:
                log.error("scanner failed: %s: %s", t, err)
    ...
    try:
        rs_7 = eng.rs_rating_asof(combined, ticker_meta, lag_days=7)
    except Exception:
    ...
    rs_movers = []
    ranked_delta = rs_delta.sort_values(ascending=False)
    top_gainers = []
    for t in ranked_delta.index:
        raw = rs_now.get(t) if hasattr(rs_now, "get") else None
        try:
            rs_here = int(raw) if raw is not None and raw == raw else 0
        except Exception:
            rs_here = 0
        if rs_here < LB_RS_LEADER_MIN:
            continue
        top_gainers.append(t)
        if len(top_gainers) >= max(RS_MOVERS_TOP_N, 10):
            break
    ...
    watch_pool = []
    for t, s in ticker_signal.items():
        if not s.get("confluence"):
            continue
        d = combined.get(t)
        if d is None:
            continue
        raw = rs_now.get(t) if hasattr(rs_now, "get") else None
        try:
            rs_val = int(raw) if raw is not None and raw == raw else None
        except Exception:
            rs_val = None
        snap = snapshot_from_df(d, rs_val)
        if not is_confluence_study({"rs": rs_val, **snap}):
            continue
        watch_pool.append((t, s, snap, rs_val))
    watch_pool.sort(key=lambda item: _watch_key((item[0], item[1])), reverse=True)
    watchlist = []
    for t, s, snap, rs_val in watch_pool[:WATCHLIST_TOP_N]:
        ...
        watchlist.append({
            ...
            "patterns": [k for k, v in s.get("rolled", {}).items() if v],
            ...
            "template_score": snap.get("template_score"),
    ...
    return {
        ...
        "universe_listed_core": LISTED_CORE, "universe_listed_full": LISTED_FULL,
        "universe_listed": universe_list_stats(),
...
def _dip_candidates(combined, ticker_meta, rs_now, rs_7, ticker_signal, limit: int | None = None) -> list:
    """ตัวนำที่กำลังพักฐาน ไม่ใช่รายชื่อซื้อ และไม่ดึงผู้แพ้มาช้อน."""
    rows = []
    if rs_now is None:
        return rows
    cap = limit if limit is not None else DIP_WATCH_TOP_N
    for t, df in combined.items():
        ...
        try:
            ...
            rs = None
        try:
            prev = int(rs_7.get(t)) if rs_7 is not None and hasattr(rs_7, "get") and rs_7.get(t) == rs_7.get(t) and rs_7.get(t) is not None else None
        except Exception:
            prev = None
        drs = (rs - prev) if rs is not None and prev is not None else 0
        snap = snapshot_from_df(df, rs)
        gate = {"rs": rs, "drs7": drs, **snap}
        if not is_leader_pause(gate):
            continue
        rolled = (ticker_signal.get(t) or {}).get("rolled") or {}
        drying = bool(rolled.get("VDU") or rolled.get("VCP"))
        off = -abs(float(snap.get("prox_52w") or 0))
        score = int(min(99, rs + min(25, abs(drs) * 2.5) + min(20, abs(off)) + (8 if drying else 0) + (10 if snap.get("stage") == 2 else 0)))
        meta = ticker_meta.get(t) or {}
        px = float(snap.get("px") or 0)
        rows.append({
            ...
            "drs7": drs,
            "off_52w": round(off, 1),
            "above50": bool(snap.get("above50")),
            "above200": bool(snap.get("above200")),
            "stage": snap.get("stage"),
            "template_score": snap.get("template_score"),
            "drying": drying,
            "score": score,
            "why": " · ".join(filter(None, [
                ...
                f"ห่างยอด {off:.0f}%",
                f"S{snap.get('stage')}",
                "เหนือ SMA200" if snap.get("above200") else "",
