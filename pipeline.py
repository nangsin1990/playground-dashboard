...
def compute_dashboard(combined, ticker_meta, fetch_results, active) -> dict:
    ...
    try:
        # ✨ FIX: เดิมใช้ eng.rs_rating_table(close_df, 7) ซึ่งเป็นคนละสูตรกับ rs_now
        # (return 7 วันเดี่ยว + จัดอันดับรวมทุกตลาด) ทำให้ drs7 เทียบกันคนละมาตรฐาน
        # ตอนนี้ใช้ rs_rating_per_market สูตรเดียวกับ rs_now เป๊ะ แค่ตัดข้อมูล 7 วันล่าสุดออก
        # ก่อนคำนวณ (offset=7) ให้ผลลัพธ์เทียบกันได้จริงแบบ apples-to-apples
        rs_7 = eng.rs_rating_per_market(combined, ticker_meta, offset=7)
    except Exception:
    ...
    for t in top_gainers:
        ...
        rs_movers.append({
            ...
            "rs": int(rs_now.get(t, 0) or 0), "drs7": int(rs_delta.get(t, 0) or 0),
            "price": last_px,
    ...
    return {
        ...
        # ✨ FIX: x["rs"] เป็น None ได้ (ticker ที่ไม่เข้าเกณฑ์ RS engine) — sort ตรงๆ กับ int
        # จะ throw TypeError ใน Python 3 ตอนนี้ treat None เป็นค่าต่ำสุดแทน กัน endpoint ล่ม
        "watchlist": sorted(watchlist, key=lambda x: x["rs"] if x["rs"] is not None else -1, reverse=True),
        "dip_watch": dip_watch,
