...
def compute_dashboard(combined, ticker_meta, fetch_results, active) -> dict:
    ...
    return {
        ...
        "feed_status": {
            "yahoo": True,
            "yahoo_status": "ok" if len(combined) >= max(1, int(len(ticker_meta) * 0.85)) else ("partial" if combined else "down"),
            "updated": now_str, "loaded": len(combined), "total": len(ticker_meta),
        },
