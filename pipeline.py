...
def compute_dashboard(combined, ticker_meta, fetch_results, active) -> dict:
    ...
    if themes_df is None or getattr(themes_df, "empty", True):
        theme_rows = []
    else:
        sort_cols = [c for c in ("r1m", "r3m") if c in themes_df.columns]
        themes = themes_df.sort_values(by=sort_cols, ascending=False) if sort_cols else themes_df
    ...
    def _watch_key(item):
        ...
        rs_val = 0
        try:
            rs_val = int(rs_now.get(t, 0) or 0)
        except Exception:
            rs_val = 0
