...
def add_technical_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # VWAP จากวอลุ่มสะสมจริงเท่านั้น — วันวอลุ่ม 0 ให้ต่อจากค่าล่าสุดที่มี
    # ห้ามแทนทั้งคอลัมน์ด้วย typical price เพราะจะทำให้สัญญาณ Above/Below VWAP หลอก
...
def _vcp_frame(df: pd.DataFrame) -> dict | None:
    ...
    score = (
        np.select([days_dry >= 10, days_dry >= 5, days_dry >= 3], [30, 18, 8], default=0)
        + np.select([dry_ratio < 20, dry_ratio < 50, dry_ratio < 80], [25, 16, 8], default=0)
        + np.select([off_peak.abs() <= 5, off_peak.abs() <= 12, off_peak.abs() <= 20], [25, 15, 8], default=0)
        + np.select([tight <= 10, tight <= 18], [20, 10], default=0)
    )
    score = pd.Series(score, index=df.index).clip(upper=99)
...
def vcp_metrics(df: pd.DataFrame) -> dict:
    ...
    if frame is None:
        return empty
    days = int(frame["days_dry"].iloc[-1] or 0)
    dry_ratio = frame["dry_ratio"].iloc[-1]
    off_peak = frame["off_peak"].iloc[-1]
    score = int(frame["score"].iloc[-1] or 0)
...
def theme_returns(close_df: pd.DataFrame, theme_map: dict, ticker_meta: dict, rs_now: pd.Series):
    ...
    for theme, tickers in theme_map.items():
        ...
        for t in valid_tickers:
            ...
            members.append({**src, "ticker": t, "r1m": round(float(r1m.get(t, 0) or 0), 2)})
...
def tech_snapshot(df: pd.DataFrame):
    ...
    vwap, vwap_sig = (None, "VWAP unavailable (no volume)") if not pd.notna(vwap_val) else (vwap_val, "N/A")
    if vwap is not None:
        if abs(float(last['Close']) - float(vwap)) < 1e-9:
            vwap_sig = "VWAP equals last print"
        else:
            vwap_sig = "Price is Above VWAP" if last['Close'] > vwap else "Price is Below VWAP"
    last_px = float(last['Close']) if pd.notna(last.get('Close')) else None
    prev_px = float(df['Close'].iloc[-2]) if len(df) > 1 and pd.notna(df['Close'].iloc[-2]) else None
    change_pct = None
...
def pivot_pack(df: pd.DataFrame) -> dict:
    empty = {"daily": None, "weekly": None, "monthly": None}
