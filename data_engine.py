...
def run_scanners(df: pd.DataFrame) -> dict:
    ...
    vcp = vcp_metrics(df)
    signals['VCP'] = pd.Series(False, index=df.index)
    signals['VCP'].iloc[-1] = bool(vcp.get("is_vcp"))
...
def vcp_metrics(df: pd.DataFrame) -> dict:
    ...
    days = 0
    for flag in dry.iloc[::-1]:
        if bool(flag):
            days += 1
        else:
            break
    last_sma = float(sma.iloc[-1]) if pd.notna(sma.iloc[-1]) else 0.0
    dry_ratio = float(vol.iloc[-1] / last_sma * 100) if last_sma > 0 else None
    peak = float(df["High"].tail(60).max())
    last_px = float(close.iloc[-1])
    off_peak = ((last_px / peak) - 1) * 100 if peak else None
    tail = close.tail(20)
    tight = ((tail.max() - tail.min()) / tail.min() * 100) if len(tail) and tail.min() > 0 else 100
    score = 0
    if days >= 10:
        score += 30
    elif days >= 5:
    ...
    if off_peak is not None:
        if abs(off_peak) <= 5:
            score += 25
        elif abs(off_peak) <= 12:
            score += 15
        elif abs(off_peak) <= 20:
            score += 8
    if tight <= 10:
        score += 20
    elif tight <= 18:
    ...
    if days >= 10 and (dry_ratio or 100) < 15:
        zone = "EXTREME DRY"
    elif days >= 5 and (dry_ratio or 100) < 50:
        zone = "DRY"
    else:
        zone = "WATCH"
    return {
        "vcp_ready": score,
        "vcp_days_dry": days,
        "vcp_dry_ratio": None if dry_ratio is None else round(dry_ratio, 1),
        "vcp_off_peak": None if off_peak is None else round(off_peak, 1),
        "vcp_target": round(peak, 2) if peak else None,
        "vcp_zone": zone,
        "is_vcp": score >= 40 and days >= 5,
    }
