...
def rs_rating_per_market(combined: dict, ticker_meta: dict, offset: int = 0) -> pd.Series:
    """
    ✨ FIX: เพิ่ม offset (จำนวนวันตัดท้ายออก) เพื่อให้คำนวณ RS "ณ N วันก่อน" ด้วยสูตร
    เดียวกันเป๊ะกับ RS ปัจจุบัน (blend 3/6/9/12M + จัดอันดับแยกตามตลาด) — เดิม caller
    ใช้ rs_rating_table(close_df, 7) แทน ซึ่งเป็นคนละสูตร (return 7 วันเดี่ยว + จัดอันดับ
    รวมทุกตลาด) ทำให้ drs7 (RS วันนี้ - RS 7 วันก่อน) เทียบกันคนละมาตรฐาน
    offset=0 (ค่าเริ่มต้น) พฤติกรรมเดิมทุกประการ
    """
