...
def _rel_excess(asset: pd.Series, bench: pd.Series, weeks: int):
    a = asset.dropna()
    ...
    left = joined.iloc[:, 0]
    right = joined.iloc[:, 1]
    if right.iloc[-1 - weeks] == 0 or left.iloc[-1 - weeks] == 0:
        return None
    ar = float(left.iloc[-1] / left.iloc[-1 - weeks] - 1)
    br = float(right.iloc[-1] / right.iloc[-1 - weeks] - 1)
    return round((ar - br) * 100, 1)


def _zscore(frame: pd.DataFrame, window: int, min_periods: int) -> pd.DataFrame:
...
def calculate_rrg_metrics(
    ...
    """JdK-style RRG: z-score of relative price and of its change, mapped to 100 ± 10 per σ, then clamped."""
    results = {}
    if benchmark_ticker not in df_weekly.columns:
        return results
    benchmark_prices = df_weekly[benchmark_ticker].ffill()
    use = [t for t in tickers if t != benchmark_ticker and t in df_weekly.columns]
    if not use:
        return results

    rel = df_weekly[use].div(benchmark_prices, axis=0).replace([np.inf, -np.inf], np.nan)
    valid_tickers = [t for t in use if rel[t].notna().sum() >= max(RRG_SMOOTHING, period + 2)]
    if not valid_tickers:
        return results
    rel = rel[valid_tickers]

    scale = 10.0
    jrs_raw = 100.0 + _zscore(rel, RRG_SMOOTHING, RRG_ROLL_MIN) * scale
    jrs_mom = jrs_raw.diff(period)
    jmo_raw = 100.0 + _zscore(jrs_mom, RRG_SMOOTHING, RRG_ROLL_MIN) * scale
    jrs_val = jrs_raw.clip(RRG_CLAMP_LO, RRG_CLAMP_HI)
    jmo_val = jmo_raw.clip(RRG_CLAMP_LO, RRG_CLAMP_HI)

    for ticker in valid_tickers:
        jrs, jmo = jrs_val[ticker].iloc[-1], jmo_val[ticker].iloc[-1]
        if not (np.isfinite(jrs) and np.isfinite(jmo)):
            continue
        tail_jrs = [float(v) for v in jrs_val[ticker].dropna().tail(tail_length).tolist() if np.isfinite(v)]
        tail_jmo = [float(v) for v in jmo_val[ticker].dropna().tail(tail_length).tolist() if np.isfinite(v)]
        n = min(len(tail_jrs), len(tail_jmo))
        tail_data = list(zip(tail_jrs[-n:], tail_jmo[-n:]))
        if not tail_data:
            tail_data = [(float(jrs), float(jmo))]
        quadrant = _classify_quadrant(jrs, jmo)
        quadrant_history = [_classify_quadrant(jr, jm) for jr, jm in tail_data]
        prev = quadrant_history[-2] if len(quadrant_history) > 1 else None
        results[ticker] = {
            "jrs": round(float(jrs), 2),
            "jmo": round(float(jmo), 2),
            "quadrant": quadrant,
            "tail": tail_data,
            "quadrant_history": quadrant_history,
            "persistence_weeks": _quadrant_persistence(quadrant_history),
            "prev_quadrant": prev,
            "just_moved": bool(prev and prev != quadrant),
            "ret_4w": _rel_excess(df_weekly[ticker], benchmark_prices, 4),
            "ret_13w": _rel_excess(df_weekly[ticker], benchmark_prices, 13),
        }
    return results
...
def resample_h4(hourly: pd.DataFrame | None, align: str = "plain") -> pd.DataFrame | None:
    """Build H4 bars from hourly.

    align='plain' — pandas 4h buckets (stock deep-dive default)
    align='comex' — America/New_York + 2h offset, buckets start 18:00 ET
    """
    if hourly is None or len(hourly) < 3:
        return None
    if not {"Open", "High", "Low", "Close"}.issubset(hourly.columns):
        return None
    ...
    try:
        ...
        if align == "comex":
            ...
            bars = df_h.resample("4h", offset="2h").agg(agg).dropna()
        else:
            bars = df_h.resample("4h").agg(agg).dropna()
        return bars if len(bars) else None
    except Exception:
        try:
            bars = hourly.resample("4h").agg(agg).dropna()
            return bars if len(bars) else None
        except Exception:
            return None


def pivot_pack(df: pd.DataFrame, hourly: pd.DataFrame | None = None, h4_align: str = "plain") -> dict:
    ...
    h4 = None
    bars = resample_h4(hourly, align=h4_align)
