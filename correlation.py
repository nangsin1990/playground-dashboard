...
def fetch_correlation() -> dict:
    ...
    close_df = pd.DataFrame(closes).sort_index().tail(CORR_PERIOD_DAYS + 5)
    try:
        rets = close_df.pct_change(fill_method=None)
    except TypeError:
        rets = close_df.pct_change()
    corr = rets.corr(min_periods=max(15, CORR_PERIOD_DAYS // 3))
    labels = [t for t in closes.keys() if t in corr.columns]
    if len(labels) < 4:
        return {"ok": False, "error": "Not enough overlapping history for correlation", "labels": []}
    ...
    for a in labels:
        ...
        matrix.append(row)
    result = {"ok": True, "labels": labels, "matrix": matrix}
    ...
    max_pair = max(pairs, key=lambda x: x["corr"]) if pairs else None
    min_pair = min(pairs, key=lambda x: x["corr"]) if pairs else None
