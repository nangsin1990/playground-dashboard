...
def fetch_rotation(mode: str = "core", market: str = "GLOBAL") -> dict:
    try:
        ...
        valid_tickers = [
            ...
            # ✨ FIX: เดิม hardcode ">10" สัปดาห์ ตอนนี้ใช้ RRG_MIN_HISTORY (30) ที่ประกาศไว้
            # ใน constants.py แต่ไม่เคยถูกอ้างถึงจริง — เกณฑ์เดิมหลวมเกินไป (แค่ ~2.5 เดือน)
            if t in df_weekly.columns and df_weekly[t].notna().sum() > RRG_MIN_HISTORY
        ]
        ...
        for ticker, data in rrg_metrics.items():
            ...
            rrg_list.append({
                ...
                "tail": tail,
                "avg_rs": avg_rs,
                # ✨ NEW: theme persistence — จำนวนสัปดาห์ติดต่อกันที่ยังอยู่ quadrant เดิม
                "persistence_weeks": data.get("persistence_weeks", 0),
                "quadrant_history": data.get("quadrant_history", []),
            })

        # เรียงให้ theme ที่ Leading ต่อเนื่องนานสุดขึ้นก่อน ช่วยดูรอบเดียวว่าตัวไหน
        # แข็งแกร่งแบบยั่งยืน ไม่ใช่แค่ Leading ชั่วคราวสัปดาห์เดียว
        rrg_list.sort(key=lambda r: (r["quadrant"] == "Leading", r["persistence_weeks"]), reverse=True)
        ...
        return {
            ...
            "rrg": rrg_list,
            # ✨ NEW: ส่ง chart bounds จาก constants เดียวกับที่ backend ใช้คำนวณ jrs/jmo
            # เดิม rotation.html hardcode {x0:90,x1:115,...} แยกไว้เอง ไม่ได้ผูกกับ backend
            "chart_bounds": {"lo": RRG_CLAMP_LO, "hi": RRG_CLAMP_HI},
        }
