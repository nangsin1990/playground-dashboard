# FILE: gold.py
"""
gold.py — Gold Command Center engine (V1.1)
Endpoint: /api/gold
...
GOLD_EVENT_CATEGORIES = {"FOMC", "MINUTES", "CPI", "NFP", "PCE", "PPI"}
...
def _pct_change_n(series: pd.Series, n: int) -> Optional[float]:
    if series is None or len(series) < n + 1:
        return None
    a, b = float(series.iloc[-1 - n]), float(series.iloc[-1])
    if a == 0:
        return None
...
def _bps_change_n(series: pd.Series, n: int) -> Optional[float]:
    ...
    if series is None or len(series) < n + 1:
        return None
    a, b = float(series.iloc[-1 - n]), float(series.iloc[-1])
    # ✨ FIX: Yahoo บางครั้งส่ง ^TNX/^IRX เป็น x10 (เช่น 42.5 แทน 4.25) — market_regime.py
    # มีการกันเคสนี้ไว้แล้ว (_yield_pct) แต่ตรงนี้ไม่เคยกัน ทำให้ tnx_pts/pullback_risk
    # เพี้ยนไป ~10 เท่าถ้า Yahoo ส่งค่าผิดสเกลมา ตอนนี้ normalize ให้ตรงกัน
    if abs(b) > 20:
        a, b = a / 10.0, b / 10.0
...
def _completed_ohlcv(df: Optional[pd.DataFrame]) -> Optional[pd.DataFrame]:
...
def _d1_structure(df: pd.DataFrame) -> tuple[str, Optional[float]]:
    ...
    ema50 = float(close.ewm(span=min(50, max(20, len(close) - 1)), adjust=False).mean().iloc[-1])
...
def _candidate_levels(df_d1: pd.DataFrame, pivots: dict, vp: dict) -> list[dict]:
    ...
    if len(df_d1) >= 60:
        add(float(df_d1["Low"].tail(60).min()), "60D Swing Low")
        add(float(df_d1["High"].tail(60).max()), "60D Swing High")
        add(float(df_d1["High"].tail(60).max()), "Prev Breakout")
        add(float(df_d1["High"].tail(60).max()), "Prev Rejection")
    if len(df_d1) >= 252:
...
def _cluster_zones(levels: list[dict], current_price: float, atr: float) -> list[dict]:
    ...
    pts = sorted(levels, key=lambda x: abs(x["price"] - current_price))
    zones = []
    for p in pts:
        placed = False
        for z in zones:
...
def _gold_macro_events() -> Optional[dict]:
    events = []
    try:
        for ds, title, subtitle in getattr(ec, "FED_MEETINGS_ALL", []):
            d = date.fromisoformat(ds)
            events.append(ec._format_event(d, "FOMC", title, subtitle, "HIGH", "federalreserve.gov"))
        for ds, title, subtitle in ec._fomc_minutes_dates():
            d = date.fromisoformat(ds)
            events.append(ec._format_event(d, "MINUTES", title, subtitle, "MEDIUM", "federalreserve.gov"))
        for row in getattr(ec, "_STATIC_MACRO", []):
            ds, cat, title, subtitle, importance, source = row
            if cat in GOLD_EVENT_CATEGORIES:
                d = date.fromisoformat(ds)
                events.append(ec._format_event(d, cat, title, subtitle, importance, source))
    except Exception:
        log.exception("gold macro events failed")
        return None
    upcoming = [e for e in events if e.get("category") in GOLD_EVENT_CATEGORIES and not e.get("is_past")]
    upcoming.sort(key=lambda e: (e.get("days_away", 999), 0 if e.get("importance") == "HIGH" else 1))
...
def _plan_for_state(state_key: str, price: float, atr: float,
    ...
    if state_key in ("BUY_ZONE", "HEALTHY_PULLBACK", "PULLBACK_RISK") and nearest_support:
...
def fetch_gold() -> dict:
    try:
        ...
        df_spot_d1 = eng.add_technical_indicators(df_spot_d1)
        ...
        df_h1_done = _completed_ohlcv(df_hourly)
        df_h4 = None
        ...
        pivots = eng.pivot_pack(df_spot_d1, hourly=df_hourly)
        vp = {}
        try:
            vp = ta._volume_profile(df_spot_d1, days=60, buckets=12) or {}
        except Exception:
            vp = {}
        ...
        gold_chg_pct = snap.get("change_pct")
        prev_d1_close = _safe_float(snap.get("prev_close"))

        quotes = gm._batch_quotes([DXY, USDTHB, TNX, VIX, GLD, GDX, SILVER, COPPER, FUT])
        coverage["quotes"] = "ok" if quotes else "missing"
        ...
        if df_fut_h is not None and len(df_fut_h) >= 30:
            done_h = _completed_ohlcv(df_fut_h)
            if done_h is not None and len(done_h) >= 25:
                slot = done_h.index[-1]
                try:
                    hour = pd.Timestamp(slot).hour
                    same = done_h[done_h.index.map(lambda x: pd.Timestamp(x).hour == hour)]["Volume"]
                    if len(same) >= 8:
        ...
        vol_ratio_used = gc_vol_h1_ratio if gc_vol_h1_ratio is not None else gc_vol_ratio
        ...
        support_zones = [z for z in all_zones if z["side"] == "support"][:3]
        resistance_zones = [z for z in all_zones if z["side"] == "resistance"][:3]
        ...
        near_support = bool(nearest_support and (current_price - nearest_support["high"]) <= atr * 1.0)
        near_resistance = bool(nearest_resistance and (nearest_resistance["low"] - current_price) <= atr * 0.75)
        ...
        breakout_level = None
        for z in all_zones:
            if _crossed_up(h1_prev, h1_curr, z["high"]) or _crossed_up(prev_d1_close, current_price, z["high"]):
                if z["side"] in ("resistance", "inside") or z["high"] >= (h1_prev or current_price):
                    breakout_level = z
                    break
        support_break_level = None
        for z in all_zones:
            if _crossed_down(h1_prev, h1_curr, z["low"]) or _crossed_down(prev_d1_close, current_price, z["low"]):
                if z["side"] in ("support", "inside") or z["low"] <= (h1_prev or current_price):
                    support_break_level = z
                    break

        vol_ok = vol_ratio_used is not None and vol_ratio_used > 1.3
        ...
        return {
            ...
            "prices": {
                ...
                "us10y": {"price": quotes.get(TNX, {}).get("price"), "chg_pct": quotes.get(TNX, {}).get("chg_pct"),
