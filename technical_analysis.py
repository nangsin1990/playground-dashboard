...
def fetch_technicals(ticker: str) -> dict:
    try:
        ...
        price_history = {
            ...
            "close":  [_safe(v) for v in df["Close"].tail(30)],
            "vwap":   [_safe(v) for v in df["VWAP"].tail(30)],
            "bb_up":  [_safe(v) for v in df["BB_UPPER"].tail(30)],
            "bb_mid": [_safe(v) for v in df["BB_MID"].tail(30)],
            "bb_lo":  [_safe(v) for v in df["BB_LOWER"].tail(30)],
        }
