    ...
    try:
        ...
        if cal is not None and not getattr(cal, "empty", True):
            ...
            if future is not None and not future.empty:
                ...
                for dt, _row in future.head(4).iterrows():
                    upcoming.append(str(dt)[:10])

        cal_pack = {}
        try:
            raw_cal = getattr(t, "calendar", None)
            if raw_cal is not None:
                if hasattr(raw_cal, "to_dict"):
                    bag = raw_cal.to_dict()
                    if isinstance(bag, dict):
                        first = next(iter(bag.values())) if bag else {}
                        if isinstance(first, dict):
                            cal_pack = {str(k): str(v)[:10] if v is not None else None for k, v in first.items()}
                        else:
                            cal_pack = {str(k): str(v)[:32] for k, v in bag.items()}
                elif isinstance(raw_cal, dict):
                    cal_pack = {str(k): str(v)[:32] for k, v in raw_cal.items() if v is not None}
        except Exception:
            cal_pack = {}
...
def fetch_dividends(ticker: str) -> dict:
    try:
        ...
        try:
            ...
            pay_date = None
            raw_pay = info2.get("dividendDate") or info2.get("lastDividendDate")
            if raw_pay:
                try:
                    if isinstance(raw_pay, (int, float)) and raw_pay > 1e9:
                        pay_date = datetime.fromtimestamp(int(raw_pay), tz=timezone.utc).strftime("%Y-%m-%d")
                    else:
                        pay_date = str(pd.Timestamp(raw_pay))[:10]
                except Exception:
                    pay_date = str(raw_pay)[:10]
        except Exception:
            div_yield = None
            div_rate = None
            ex_div   = None
