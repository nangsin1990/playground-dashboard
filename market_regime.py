...
from constants import CACHE_TTL_DATA as CACHE_TTL
...
def _yield_pct(series: pd.Series) -> float | None:
    """Yahoo บางครั้งส่ง ^TNX/^IRX เป็น x10."""
...
def _sig_momentum(close: pd.Series):
    ...
    r1s = f"{r1m:+.1f}%" if r1m is not None else "–"
    r3s = f"{r3m:+.1f}%" if r3m is not None else "–"
...
def _sig_volatility(vix_close: pd.Series):
    ...
    # Rising VIX = bad
    if vix20 is not None and vix20 > 20:
        score -= 2
    # ✨ FIX: เดิม score ต่ำสุดได้ -12 (vix>=40 ให้ -10 แล้วลบซ้ำอีก -2) เกินขอบเขต ±10
    # ที่ signal อื่นทุกตัวยึดไว้ (ดู docstring ด้านบน) ทำให้ composite/normalized
    # เกิน ±100 ได้เล็กน้อยแบบไม่ตั้งใจ ตอนนี้ clamp ให้อยู่ในขอบเขตเดียวกัน
    score = max(-10.0, min(10.0, score))
    vix20s = f"{vix20:+.0f}%" if vix20 is not None else "–"
...
def _sig_credit(hyg: pd.Series, lqd: pd.Series):
    ...
    rs = f"{ratio_roc:+.2f}%" if ratio_roc is not None else "–"
...
def _sig_yield_curve(t10y: pd.Series, t3m: pd.Series):
    ...
    y10 = _yield_pct(t10y["Close"])
    y3m = _yield_pct(t3m["Close"])
...
def compute_market_regime(
    ...
    # Confidence: สัญญาณที่เป็นกลาง (0) ไม่นับเข้าฝั่งใด
    signed = [s[0] for s in signals_raw if s[0] != 0]
    if not signed:
        confidence = 0
    else:
        if normalized >= 0:
            confidence_raw = sum(1 for x in signed if x > 0) / len(signed)
        else:
            confidence_raw = sum(1 for x in signed if x < 0) / len(signed)
    ...
    return {
        ...
        "breadth_us_ma50":  round(breadth_us_ma50, 1) if breadth_us_ma50 is not None else None,
        "breadth_us_ma200": round(breadth_us_ma200, 1) if breadth_us_ma200 is not None else None,
    }
