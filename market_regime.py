...
def _roc(series: pd.Series, n: int) -> float | None:
    """n-period rate of change: last value vs value n bars earlier (n intervals)."""
    ...
    try:
        ...
        return None
    if prev_f == 0 or last_f != last_f or prev_f != prev_f:
        return None
    return (last_f / prev_f - 1) * 100
...
def _sig_volatility(vix_close: pd.Series):
    ...
    # Rising VIX = bad
    if vix20 is not None and vix20 > 20: score -= 2
...
def _sig_copper_gold(copper: pd.Series, gold: pd.Series):
    ...
    rs = f"{cg_roc:+.2f}%" if cg_roc is not None else "–"
