...
def fetch_event_impact() -> dict:
    try:
        ...
        raw = yf.download(tickers, period="2y", auto_adjust=True, progress=False, timeout=45)
        ...
        for code, items in by_code.items():
            ...
            past_dates = {pd.Timestamp(e["date"]) for e in past if not e.get("approx")}
            asset_rows = []
            vol_rows = []
            if approx:
            ...
            for label, tk in ASSETS.items():
                ...
                event_mask = series.index.normalize().isin(past_dates)
        ...
        return {"ok": False, "error": "event_impact_unavailable"}
