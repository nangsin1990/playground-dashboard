...
try:
    ...
    from .utils import fmt, safe_get, strip_tz, normalize_debt_to_equity
...
class MarketWinnerScanner:
    ...
    def fetch_data(self) -> bool:
        try:
            self.info = {}
            for getter in (
                (lambda: self.stock.get_info()) if hasattr(self.stock, "get_info") else None,
                lambda: self.stock.info,
            ):
                if getter is None:
                    continue
                try:
            ...
            if not self.info or (
                ...
                try:
                    ...
                    self.info["currentPrice"] = getattr(fi, "last_price", None) or getattr(fi, "lastPrice", None)
                    self.info["marketCap"] = getattr(fi, "market_cap", None) or getattr(fi, "marketCap", None)
                except Exception:
                    pass
    ...
    def score_sections(self):
        ...
        labels = dual_labels(q["score"], m["score"], quality_available=bool(q.get("available", True)))
    ...
    def decide(self) -> Dict[str, Any]:
        """Single decision source — same engine as payload / stock_check."""
        if not getattr(self, "scorecards", None):
            self.score_sections()
        cards = self.scorecards or {}
        extra = self.extra or {}
        fund = getattr(self, "_fund", {}) or {}
        return evaluate_decision(
            dq=float(extra.get("data_quality_overall") or 0),
            quality=int((cards.get("quality") or {}).get("score") or 0),
            momentum=int((cards.get("momentum") or {}).get("score") or 0),
            valuation=int((cards.get("valuation") or {}).get("score") or 0),
            risk=int((cards.get("risk") or {}).get("score") or 0),
            tech_verdict=(cards.get("momentum") or {}).get("tech_verdict", ""),
            stage=(cards.get("momentum") or {}).get("stage", ""),
            rs_verdict=(cards.get("momentum") or {}).get("rs_verdict", ""),
            avg_rs=(cards.get("momentum") or {}).get("avg_rs"),
            price=getattr(self, "_price", None),
            mas=getattr(self, "_mas", {}) or {},
            rsi=(getattr(self, "_tech", {}) or {}).get("RSI"),
            regime=getattr(self, "_regime", "") or "",
            days_to_earnings=extra.get("days_to_earnings"),
            fcf=fund.get("free_cashflow"),
            de_norm=fund.get("debt_to_equity_norm"),
            quality_available=bool((cards.get("quality") or {}).get("available", True)),
            valuation_available=bool((cards.get("valuation") or {}).get("available", True)),
        )
    ...
    def classification(self, score: int, data_quality: float, quality: int, momentum: int) -> str:
        ...
        labels = {
            ...
            "CANDIDATE": "FILTER CANDIDATE (research only)",
    ...
    def final_signal_bundle(self, score: int, data_quality: float) -> Dict[str, str]:
        ...
        if price and mas.get("MA20") and price > mas["MA20"] * 1.12:
            overextended = True
        if d.get("stance") == "INSUFFICIENT" or data_quality < DQ_GATE:
            return {
                "signal": "⚪ NO SIGNAL",
