...
def health_data(mode: str = Query("core")):
    try:
        ...
        quality = dash.get("data_quality") or {}
        ready = bool(dash.get("ok")) and float(quality.get("coverage_pct") or 0) >= 50
        return {
            "status": "ok" if ready else "degraded",
            "check": "data",
            "ready": ready,
            "feed_status": dash.get("feed_status"),
            "data_quality": quality,
            "updated": dash.get("updated"),
        }
    except Exception:
        log.exception("health_data failed")
        return JSONResponse(status_code=503, content={"status": "down", "check": "data", "ready": False})
