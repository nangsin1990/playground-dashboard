...
def status():
    ...
    try:
        from universe import universe_list_stats
        universe_info = universe_list_stats()
    except Exception as e:
        universe_info = {"error": str(e)}
    return {
        "status": "ok",
        "booted": _boot_time,
...
def dashboard(mode: str = Query("core"), market: Optional[str] = None, _: None = Depends(get_cache_clearer(_clear_price_and_pack))):
    ...
    try:
        ...
        log.exception("my_watchlist failed")
        result["my_watchlist"] = {"ok": False, "error": str(e), "rows": [], "alerts": [], "loaded": 0, "total": 0}
    return _resp(result)
...
def _clear_gold():
    bust = getattr(gd, "_bust_gold_cache", None)
    cached = getattr(gd, "fetch_gold_cached", None)
    if callable(bust) and cached is not None:
        try:
            bust(cached)
            return
        except Exception:
            pass
    for name in ("fetch_gold_cached", "fetch_gold"):
        fn = getattr(gd, name, None)
        cb = getattr(fn, "cache_clear", None) if fn is not None else None
        if callable(cb):
            try:
                cb()
            except Exception:
...
@app.get("/api/gold")
def gold_api(_: None = Depends(get_cache_clearer(_clear_gold))):
...
@app.get("/api/profile")
def profile_api(ticker: str, _: None = Depends(get_cache_clearer(spf.fetch_analyst.cache_clear))):
    return _resp(spf.fetch_analyst(ticker=ticker))


@app.get("/api/holders")
def holders_api(ticker: str, _: None = Depends(get_cache_clearer(spf.fetch_holders.cache_clear))):
    return _resp(spf.fetch_holders(ticker=ticker))


@app.get("/api/news")
def news_api(ticker: str, _: None = Depends(get_cache_clearer(spf.fetch_news.cache_clear))):
    return _resp(spf.fetch_news(ticker=ticker))
...
def stock_check_api(
    ...
    try:
        ...
        log.exception("stock_check endpoint failed ticker=%s", ticker)
        data = {"ok": False, "ticker": ticker, "error": "ตรวจสภาพไม่สำเร็จ", "detail": str(exc)[:180]}
    if not isinstance(data, dict):
        data = {"ok": False, "ticker": ticker, "error": "ผลตรวจใช้ไม่ได้"}
    # อย่าส่ง 503 — หน้าเว็บจะได้ JSON เสมอ แม้สรุปไม่ครบ
    return JSONResponse(_jsonable(data), status_code=200)
