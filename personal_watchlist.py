...
def build_my_watchlist(combined: dict, ticker_meta: dict, ticker_signal: dict,
    ...
    for t in PERSONAL_TICKERS:
        ...
        if key is not None and rs_now is not None:
            try:
                raw_rs = rs_now.get(key, 0)
                rs = int(raw_rs) if raw_rs == raw_rs and raw_rs is not None else 0
            except Exception:
                rs = 0
        ...
        if loaded:
            try:
                ...
                price = round(last, 4 if last < 1 else 2)
            except Exception:
                pass
        ...
        if earn and t not in ETF_SKIP_EARNINGS and int(earn.get("days_away") or 99) <= 7:
