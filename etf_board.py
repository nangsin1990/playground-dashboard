...
def _batch_history(tickers: list[str], period="2y") -> dict[str, pd.DataFrame]:
...
def _blended_return(close: pd.Series) -> float:
    """40% 3M + 20% 6M + 20% 9M + 20% 12M — ขาที่ข้อมูลไม่พอตัดน้ำหนักออก ไม่คิดเป็น 0."""
    legs = ((63, 0.4), (126, 0.2), (189, 0.2), (252, 0.2))
    num = den = 0.0
...
def fetch_etf_board() -> dict:
    ...
    history = _batch_history(ETF_TICKERS, period="2y")
    ...
    for sym in ETF_TICKERS:
        ...
        idx = pd.DatetimeIndex(pd.to_datetime(df.index, utc=True, errors="coerce")).tz_localize(None)
        ytd_close = close.copy()
        ytd_close.index = idx
        ytd_close = ytd_close[ytd_close.index.year == datetime.now().year].dropna()
        r_ytd = None
        if len(ytd_close) >= 2 and float(ytd_close.iloc[0]):
            r_ytd = round((float(ytd_close.iloc[-1]) / float(ytd_close.iloc[0]) - 1) * 100, 2)
