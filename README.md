...
สถานะ: ใช้ศึกษา/วิจัยได้ แต่ **ยังไม่มี auth ใดๆ ในซอร์สชุดนี้** (`backend.py` ไม่เช็ค API key)
ถ้าจะเปิดผ่าน ngrok/public ให้ user อื่นเข้าถึงได้ ต้องเพิ่ม auth layer เองก่อน — ดูหัวข้อ
"ช่องโหว่ที่ยังไม่ได้แก้" ด้านล่าง
...
## ช่องโหว่ที่ยังไม่ได้แก้ (สำคัญถ้าจะเปิด public)

- ไม่มี `security.py` — ไม่มีการเช็ค API key/auth บน endpoint ใดเลย
- Env vars ที่ README เวอร์ชันก่อนอ้างถึง (`DASHBOARD_API_KEY`, `DASHBOARD_ADMIN_KEY`,
  `DASHBOARD_CORS_ORIGINS`, `DASHBOARD_ALLOW_TUNNELS`, `DASHBOARD_CACHE_SECRET`)
  **ไม่ถูกอ่านโดยโค้ดจริง** — ตั้งแล้วไม่มีผลอะไร
- `app.add_middleware(CORSMiddleware, allow_origins=["*"], ...)` เปิดกว้างทุก origin
- ถ้าจะเปิด ngrok ให้คนนอกเข้าถึงได้ อย่างน้อยควรใส่ ngrok basic auth
  (`ngrok http 8000 --basic-auth "user:pass"`) เป็นชั้นป้องกันชั่วคราว จนกว่าจะมี auth
  layer จริงในโค้ด
...
`backend.py` มี background thread (`_prewarm_loop`, เริ่มตอน FastAPI `startup` event)
ยิง `pipeline.load_market_pack("full")` และ `("core")` ล่วงหน้าทุก `PREWARM_INTERVAL_SEC`
(ตั้งไว้ 14 นาที ใน `constants.py`, สั้นกว่า `CACHE_TTL_DATA` 15 นาทีเล็กน้อยเพื่อ refresh
ก่อนหมดอายุ) — ทำให้ user คนแรกที่เปิด `mode=full` ไม่ต้องรอ cold-load ~900 ticker เอง
