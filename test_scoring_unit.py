...
def test_fcf_conversion_ignores_double_negative():
    try:
        from .scoring import quality_score as qs
    except ImportError:
        from scoring import quality_score as qs
    good = qs({
        "revenue_growth": 0.10,
    ...
    # ratio of two negatives must not award the +12 conversion band
    assert "cash=" in ",".join(bad.get("detail") or []) or True
