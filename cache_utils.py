...
_CACHE_MAGIC = b"PGCH1"
_CACHE_SECRET = os.environ.get("DASHBOARD_CACHE_SECRET", "playground-local-cache").encode()


def _sign(payload: bytes) -> bytes:
    return hmac.new(_CACHE_SECRET, payload, hashlib.sha256).digest()


def _pack(value: Any) -> bytes:
    raw = pickle.dumps(value, protocol=pickle.HIGHEST_PROTOCOL)
    return _CACHE_MAGIC + _sign(raw) + raw


def _unpack(blob: bytes) -> Any:
    if not blob.startswith(_CACHE_MAGIC):
        raise ValueError("unsigned cache rejected")
    digest = blob[len(_CACHE_MAGIC):len(_CACHE_MAGIC) + 32]
    raw = blob[len(_CACHE_MAGIC) + 32:]
    if not hmac.compare_digest(digest, _sign(raw)):
        raise ValueError("cache signature mismatch")
    return pickle.loads(raw)


# ── Core disk ops (signed pickle) ─────────────────────────────────────────────
def _disk_load(path: Path, ttl_seconds: float) -> tuple[bool, Any]:
    """
    Load from disk. Returns (hit, value).
    hit=False if file missing, expired, unsigned, or corrupt.
    """
    if not path.exists():
        return False, None
    try:
...
def _disk_save(path: Path, value: Any) -> None:
    """Write signed value to disk atomically (tmp → rename)."""
