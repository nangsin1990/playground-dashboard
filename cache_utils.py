...
# ── Core disk ops (pickle แบบต้นฉบับ เพื่อใช้ cache เดิมบน Drive ได้) ──
...
def ttl_cache(ttl_seconds: float):
    ...
    def decorator(func):
        ...
        lock      = threading.Lock()
        compute_locks: dict[tuple, threading.Lock] = {}
        func_name = func.__name__

        def _get_compute_lock(key: tuple) -> threading.Lock:
            with lock:
                lk = compute_locks.get(key)
                if lk is None:
                    lk = threading.Lock()
                    compute_locks[key] = lk
                return lk

        def _mem_hit(key: tuple):
            now = time.time()
            with lock:
        ...
        def wrapper(*args, **kwargs):
            ...
            # singleflight: ถ้าอีก request กำลังคำนวณ key เดียวกัน ให้รอผล ไม่ดึง Yahoo ซ้ำ
            clk = _get_compute_lock(key)
            with clk:
                cached = _mem_hit(key)
                if cached is not None:
                    return cached
                ...
                threading.Thread(target=_disk_save, args=(disk_path, val), daemon=True).start()
                return val
