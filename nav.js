...
function coldStartBanner() {
  ...
  let timer = null;
  return {
    start() { if (timer) clearTimeout(timer); timer = setTimeout(() => banner.style.display = "block", 3000); },
...
/** Colored RS badge */
function rsBadge(rs) {
  if (rs == null || rs === "" || isNaN(Number(rs))) {
    return `<span style="background:#e2e8f0;color:#64748b;padding:1px 6px;border-radius:4px;font-size:11px">—</span>`;
  }
  const n = Number(rs);
  const c = n >= 80 ? "#10b981" : n >= 60 ? "#f59e0b" : n >= 40 ? "#6b7280" : "#ef4444";
  return `<span style="background:${c}22;color:${c};padding:1px 6px;border-radius:4px;font-size:11px">${n}</span>`;
}
  ...
  window.fetch = function (...args) {
    ...
    return _fetch.apply(this, args)
      .then(resp => {
        ...
        cs.stop();
        setLoading(false);
        // ✨ FIX: เดิมส่ง `clone` (ทั้ง Response object) เข้า showLastUpdated ซึ่งข้างในเช็ค
        // `typeof x.get === "function"` (เผื่อเป็น Headers) — Response ไม่มี .get เอง เลย
        // false เสมอ แล้วตกไปเช็ค `.updated` ที่ก็ไม่มีบน Response เหมือนกัน → badge
        // "🕐 last updated" เลยว่างตลอดทุกหน้า อีกทั้ง backend ก็ไม่เคยส่ง header
        // X-Data-Updated มาด้วย ตอนนี้อ่านค่า `updated` จาก JSON body ของ response แทน
        clone.json().then(data => {
          if (data && typeof data === "object" && data.updated) {
            showLastUpdated(data.updated);
          }
        }).catch(() => { /* response ไม่ใช่ JSON (เช่น .html/.css) — ข้ามไป */ });
        return resp;
      })
      .catch(err => {
        cs.stop();
