...
def fetch_macro_calendar() -> dict:
    """FOMC + FRED/static เท่านั้น ไม่ยิง yfinance รายตัว"""
    today = date.today()
    cutoff_start = today - timedelta(days=LOOK_BACK_DAYS)
    cutoff_end   = today + timedelta(days=LOOK_AHEAD_DAYS)
    events: list[dict] = []
    ...
    if not fred_ok:
        used_fallback = True
        for ev in _static_fallback(cutoff_start, cutoff_end):
            ev["is_fallback"] = True
            ev["source_status"] = "fallback"
            events.append(ev)
    ...
    return {
        ...
        "total": min(len(unique), MAX_EVENTS),
        "next_fomc": next((e for e in unique if e["category"] == "FOMC" and not e["is_past"]), None),
        "fred_connected": fred_ok,
        "yf_earnings_connected": False,
        "is_fallback": used_fallback,
        "source_status": "live" if fred_ok else "fallback",
        "last_verified_at": now.isoformat(timespec="seconds"),
        "warning": None if fred_ok else "ใช้ปฏิทินสำรองที่ hard-code — วันที่จริงอาจถูกเลื่อน",
    }
...
def fetch_economic_calendar() -> dict:
    ...
    # E. Static fallback if FRED unreachable — ไม่ถือว่าเป็น live calendar
    used_fallback = False
    if not fred_ok:
        used_fallback = True
        for ev in _static_fallback(cutoff_start, cutoff_end):
