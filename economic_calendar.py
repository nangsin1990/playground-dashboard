...
def _norm_session(raw) -> str | None:
    s = str(raw or "").strip().upper()
    if "BMO" in s or "BEFORE" in s or s in {"BTO", "PRE"}:
        return "BMO"
    if "AMC" in s or "AFTER" in s or s in {"AMC", "POST"}:
        return "AMC"
    return None


def load_ticker_earnings(raw_sym: str) -> list[dict]:
    """แหล่งวันงบชุดเดียว ให้ปฏิทินกับหน้า earnings ใช้ร่วมกัน."""
    try:
        import pandas as pd
        import yfinance as yf
    except ImportError:
        return []
    yf_sym = (raw_sym or "").replace(".", "-")
    if not yf_sym:
        return []
    out = []
    try:
        ...
        if edf is not None and hasattr(edf, "index") and len(edf.index):
            ...
            for _, row in frame.iterrows():
                ...
                if ed is None:
                    continue
                sess = _norm_session(row.get("Event Type") or row.get("Time")) or _session_flag(row.get(idx_col))
                est = _num_or_none(row.get("EPS Estimate"))
                act = _num_or_none(row.get("Reported EPS"))
                sur = _num_or_none(row.get("Surprise(%)"))
                beat = None
                if sur is not None:
                    beat = sur > 0
                elif est is not None and act is not None:
                    beat = act > est
                out.append({
                    "date_obj": ed,
                    "ticker": raw_sym,
                    "eps_est": est,
                    "eps_actual": act,
                    "surprise": None if sur is None else round(sur, 1),
                    "beat": beat,
                    "session": sess,
                })
        if not out:
        ...
        return out
    seen, uniq = set(), []
    for e in sorted(out, key=lambda x: x["date_obj"]):
...
def _fetch_earnings_yf() -> list[dict]:
    ...
    for raw_sym in _earnings_universe():
        for e in load_ticker_earnings(raw_sym):
            if cutoff_start <= e["date_obj"] <= cutoff_end:
                events.append(e)
    seen, out = set(), []
    for e in events:
        key = (e["date_obj"], e["ticker"])
