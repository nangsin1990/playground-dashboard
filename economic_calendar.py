...
_HIGH_IMPACT   = {"AAPL","MSFT","NVDA","GOOG","GOOGL","META","AMZN","TSLA","JPM","AVGO","TSM"}
...
def _fetch_earnings_yf() -> list[dict]:
    ...
    events = []
    for raw_sym in EARNINGS_WATCHLIST:
        if raw_sym in _ETF_BLACKLIST:
            continue
        ...
        try:
            ...
            for d in dates:
                ...
                if cutoff_start <= ed <= cutoff_end:
                    events.append({"date_obj": ed, "ticker": raw_sym, "eps_est": ""})
        except Exception:
            continue
    seen, out = set(), []
    for e in events:
...
def next_gold_macro_event() -> Optional[dict]:
    """Next high-impact macro print that moves gold. Public API for gold.py.

    Uses the same calendar as the Calendar page so dates do not drift.
    """
    upcoming: list[dict] = []
    try:
        pack = fetch_economic_calendar()
        if pack and pack.get("ok"):
            upcoming = [
                e for e in (pack.get("events") or [])
                if e.get("category") in GOLD_EVENT_CATEGORIES and not e.get("is_past")
            ]
    except Exception:
        upcoming = []
    ...
    if not upcoming:
        try:
            ...
            for ds, title, subtitle in FED_MEETINGS_ALL:
                rows.append((ds, "FOMC", title, subtitle, "HIGH", "federalreserve.gov"))
            for ds, title, subtitle in _fomc_minutes_dates():
                rows.append((ds, "MINUTES", title, subtitle, "MEDIUM", "federalreserve.gov"))
            extra = list(_STATIC_MACRO)
            try:
                extra = extra + _extend_static_macro(cutoff_end)
            except Exception:
                pass
            for row in extra:
                ds, cat, title, subtitle, importance, source = row
                if cat in GOLD_EVENT_CATEGORIES:
                    rows.append((ds, cat, title, subtitle, importance, source))
            for ds, cat, title, subtitle, importance, source in rows:
                d = date.fromisoformat(ds)
                if d >= today:
                    upcoming.append(_format_event(d, cat, title, subtitle, importance, source))
        except Exception:
            return None
...
def _fetch_economic_calendar() -> dict:
    ...
    return {
        ...
        "source_status": "fred" if fred_ok else "fallback",
        "is_fallback": not fred_ok,
    }
...
def _approx_month_events(y: int, m: int) -> list[tuple]:
    """วันที่ประมาณสำหรับเดือนที่ยังไม่มีใน _STATIC_MACRO — ใช้ pattern เดียวกับ event_impact.py
    (NFP = ศุกร์แรกของเดือน, CPI ~วันที่ 12, PCE ~วันที่ 28) ระบุ 'estimated' ไว้ใน subtitle
    เพื่อไม่ให้สับสนกับวันที่จริงที่ยืนยันแล้วในลิสต์ hardcode ด้านบน."""
    pce_day = 27 if m == 2 else 28
    return [
        (_first_friday(y, m).isoformat(), "NFP", "Nonfarm Payrolls",
         f"{y}-{m:02d} Jobs Report (estimated)", "HIGH", "bls.gov"),
        (date(y, m, 12).isoformat(), "CPI", "Consumer Price Index",
         f"{y}-{m:02d} CPI (estimated)", "HIGH", "bls.gov"),
        (date(y, m, pce_day).isoformat(), "PCE", "PCE / Personal Income",
         f"{y}-{m:02d} PCE (estimated)", "HIGH", "bea.gov"),
    ]


def _extend_static_macro(cutoff_end: date) -> list[tuple]:
    """✨ FIX: _STATIC_MACRO (hardcode) หมดข้อมูลประมาณ ก.ย.-ต.ค. 2026 — ถ้าไม่มี
    FRED_API_KEY ตั้งไว้และไม่มีใครมาอัปเดตลิสต์ hardcode ปฏิทินจะเงียบๆ ว่างเปล่าไปเรื่อยๆ
    หลังจากนั้น ตอนนี้ต่อคิวเดือนถัดจากวันที่ล่าสุดใน _STATIC_MACRO ด้วยวันที่ประมาณการ
    จนกว่าจะถึง cutoff_end กัน static fallback หมดอายุแบบไม่มีใครรู้ตัว."""
    if not _STATIC_MACRO:
        return []
    last_date = max(date.fromisoformat(row[0]) for row in _STATIC_MACRO)
    y, m = last_date.year, last_date.month + 1
    if m > 12:
        y, m = y + 1, 1
    out: list[tuple] = []
    while date(y, m, 1) <= cutoff_end:
        out.extend(_approx_month_events(y, m))
        m += 1
        if m > 12:
            y, m = y + 1, 1
    return out
...
def _static_fallback(cutoff_start: date, cutoff_end: date) -> list[dict]:
    out = []
    all_rows = _STATIC_MACRO + _extend_static_macro(cutoff_end)
